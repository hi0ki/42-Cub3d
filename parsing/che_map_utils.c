...
/*   Updated: 2024/09/06 08:57:14 by eel-ansa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!map[y][x + 1] || !ft_strchr("1WESN0P", map[y][x + 1]))
		return (-1);
	if (!ft_strchr("1WESN0P", map[y][x - 1]))
		return (-1);
	if (y > 0 && (!ft_strchr("1WESN0P", map[y - 1][x])))
		return (-1);
	if (y != ft_lenarray(map) - 1 && (!ft_strchr("1WESN0P", map[y + 1][x])))
		return (-1);
...
int found_zero_index(char **arr, t_map *s_map)
{
	...
	while (arr[y])
	{
		...
		while (arr[y][x])
		{
			if (arr[y][x] == '0' || ft_strchr("WESNP", arr[y][x]))
			{
				s_map->player.x = x;
				s_map->player.y = y;
				return (1);
