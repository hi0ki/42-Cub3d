...
static int init_gun(t_gun *gun, t_data *data, int in)
{
    ...
    if (gun->height * gun->scale_factor > HEIGHT || gun->width * gun->scale_factor > WIDTH)
    {
...
static void draw_scaled_pixel(t_data *data, t_gun *gun)
{
    ...
    while (gun->k < gun->scale_factor)
    {
        ...
        {
            mlx_put_pixel(data->image, gun->x_c + (gun->j * gun->scale_factor) + gun->l,
                          gun->y_c + (gun->i * gun->scale_factor) + gun->k, color_pixel(gun->arr));
            gun->l++;
        }
        gun->k++;
    }
